...
export default function GradingBySectionPage() {
  ...
  const [step, setStep] = useState<"level" | "grade" | "classroom" | "students" | "config" | "grades">("level")
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Configura la estructura de evaluación e ingresa las calificaciones de tus estudiantes
          </p>
        </div>

        {/* Pasos */}
        <div className="mb-8 flex gap-2 overflow-x-auto pb-2">
          {["level", "grade", "classroom", "students", "config", "grades"].map((s, i) => (
            <div key={s} className="flex items-center">
              <div
                className={`px-4 py-2 rounded-full font-semibold text-sm whitespace-nowrap transition-colors ${
                  step === s
                    ? "bg-sky-600 text-white"
                    : ["level", "grade", "classroom", "students", "config", "grades"].indexOf(step) > i
              ...
              {i < 5 && <div className="w-2 h-0.5 bg-gray-300 mx-1" />}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
          ...
          {step === "config" && (
            <DistributionConfig onComplete={handleConfigComplete} onBack={() => setStep("students")} />
          )}
